...
public class MainActivityV2 extends ComponentActivity {
    ...
    private void injectNativeFeatures() {
        String js =
            ...
            + "function workRoute(){var m=(location.hash||'').match(/#\\/obra\\/([^\\/?]+)/);return m?decodeURIComponent(m[1]):'';}"
            ...
            + "function findWorkTitle(){var info=chapterInfo(),wid=info?info.work:workRoute();if(wid){var saved=sessionStorage.getItem('mpNativeWorkTitle:'+wid);if(saved)return saved;}var selectors=['[data-work-title]','.reader-work-title','.reader-book-title','.chapter-work-title','.work-title','.obra-title','.series-title'];for(var i=0;i<selectors.length;i++){var e=document.querySelector(selectors[i]);if(e&&clean(e.textContent))return clean(e.textContent);}var link=document.querySelector('a[href*=\"#/obra/\"],a[href*=\"/obra/\"]');if(link&&clean(link.textContent))return clean(link.textContent);if((location.hash||'').indexOf('#/obra/')===0){var h=document.querySelector('h1,h2');if(h&&clean(h.textContent))return clean(h.textContent);}return 'Obra MP SCAN';}"
            + "function rememberWorkTitle(){var wid=workRoute();if(!wid)return;var t=findWorkTitle();if(t&&t!=='Obra MP SCAN')sessionStorage.setItem('mpNativeWorkTitle:'+wid,t);}"
            + "function hideWholeWorkDownloads(){document.querySelectorAll('a,button,[role=button]').forEach(function(el){if(el.id==='mp-native-downloads'||el.id==='mp-native-chapter-download'||el.classList.contains('mp-native-ch-download'))return;var t=clean(el.textContent).toLowerCase();var whole=(t.indexOf('baixar obra')>=0||t.indexOf('baixar a obra')>=0||t.indexOf('download da obra')>=0||t.indexOf('download obra')>=0||t.indexOf('baixar tudo')>=0||t.indexOf('baixar todos os capítulos')>=0||t.indexOf('baixar todos os capitulos')>=0);if(whole)el.style.setProperty('display','none','important');});}"
            + "if(window.__mpNativeOfflineInstalled){hideWholeWorkDownloads();rememberWorkTitle();if(window.MPScanNativeRefresh)window.MPScanNativeRefresh();return;}"
            + "window.__mpNativeOfflineInstalled=true;"
            + "var css=document.createElement('style');css.id='mp-native-offline-style';css.textContent='"
            ...
            + "+'@media(max-width:520px){#mp-native-downloads,#mp-native-chapter-download{bottom:76px;padding:9px 12px;font-size:11px}.mp-native-ch-download{padding:6px 8px;font-size:10px}}';document.head.appendChild(css);"
            + "var p=document.createElement('div');p.id='mp-native-progress';document.body.appendChild(p);"
            + "window.MPScanNativeUI={progress:function(t){var e=document.getElementById('mp-native-progress');if(!e)return;e.textContent=t;e.style.display='block';},done:function(t){var e=document.getElementById('mp-native-progress');if(!e)return;e.textContent=t||'Capítulo salvo para leitura offline ✓';setTimeout(function(){e.style.display='none';},2600);window.MPScanNativeRefresh&&window.MPScanNativeRefresh();},fail:function(t){var e=document.getElementById('mp-native-progress');if(!e)return;e.textContent=t||'Não foi possível baixar o capítulo.';e.style.display='block';setTimeout(function(){e.style.display='none';},3600);}};"
            ...
            + "function pageSources(){var selectors=['#chapterReaderContent img','#readerPages img','.reader-pages img','#readerStage img','.reader-stage img','.chapter-reader img','.reader-content img','.reader-images img','.reader-view img','[data-reader-pages] img'];var imgs=[];selectors.forEach(function(s){document.querySelectorAll(s).forEach(function(i){imgs.push(i);});});if(!imgs.length){document.querySelectorAll('main img,article img,#app img').forEach(function(i){var h=i.naturalHeight||i.height||0,w=i.naturalWidth||i.width||0,src=i.currentSrc||i.src||'';if((h>520&&w>220)||src.indexOf('data:image/')===0)imgs.push(i);});}var seen={},out=[];imgs.forEach(function(i){var s=i.currentSrc||i.src||'';if(!s||seen[s])return;seen[s]=1;out.push(s);});return out;}"
            + "function chapterTitle(){var selectors=['[data-chapter-title]','.reader-chapter-title','.chapter-title','.reader-title'];for(var i=0;i<selectors.length;i++){var e=document.querySelector(selectors[i]);if(e&&clean(e.textContent))return clean(e.textContent);}var h=document.querySelector('#chapterReaderContent h1,#chapterReaderContent h2,h1,h2');return h&&clean(h.textContent)?clean(h.textContent):(document.title||'Capítulo MP SCAN');}"
            + "function downloadNow(){var info=chapterInfo();if(!info)return;var pages=pageSources();if(!pages.length){window.MPScanNativeUI.fail('As páginas ainda não carregaram. Tente novamente em alguns segundos.');return;}var wt=findWorkTitle();if(wt&&wt!=='Obra MP SCAN')sessionStorage.setItem('mpNativeWorkTitle:'+info.work,wt);window.MPScanNativeUI.progress('Preparando '+pages.length+' páginas…');MPScanApp.downloadChapter(info.work,info.chapter,wt,chapterTitle(),JSON.stringify(pages));}"
            ...
            + "function addGlobalButton(){var b=document.getElementById('mp-native-downloads');if(!b){b=document.createElement('button');b.id='mp-native-downloads';b.type='button';b.textContent='📚 Capítulos baixados';b.onclick=function(){MPScanApp.openDownloads();};document.body.appendChild(b);}}"
            + "function addChapterButton(){var info=chapterInfo(),b=document.getElementById('mp-native-chapter-download');if(!info){if(b)b.remove();return;}if(!b){b=document.createElement('button');b.id='mp-native-chapter-download';b.type='button';b.onclick=downloadNow;document.body.appendChild(b);}b.textContent=MPScanApp.isChapterDownloaded(info.work,info.chapter)?'✓ Capítulo salvo':'⬇ Baixar capítulo';}"
            + "function addWorkButtons(){if((location.hash||'').indexOf('#/obra/')!==0)return;rememberWorkTitle();var wid=workRoute(),wt=findWorkTitle(),used={};document.querySelectorAll('a[href*=\"/capitulo/\"],[data-route*=\"/capitulo/\"],[data-chapter-id]').forEach(function(a){var route=a.getAttribute('data-route')||a.getAttribute('href')||'';var h=hashFor(route);if(h.indexOf('#/capitulo/')!==0){var cid=a.getAttribute('data-chapter-id');if(cid&&wid)h='#/capitulo/'+encodeURIComponent(wid)+'/'+encodeURIComponent(cid);}if(h.indexOf('#/capitulo/')!==0||used[h])return;used[h]=1;var parent=a.parentElement||a;var exists=false;if(parent.querySelectorAll)parent.querySelectorAll('.mp-native-ch-download').forEach(function(x){if(x.getAttribute('data-target')===h)exists=true;});if(exists)return;var b=document.createElement('button');b.type='button';b.className='mp-native-ch-download';b.setAttribute('data-target',h);var m=h.match(/#\\/capitulo\\/([^\\/]+)\\/([^\\/?]+)/),downloaded=false;if(m){try{downloaded=MPScanApp.isChapterDownloaded(decodeURIComponent(m[1]),decodeURIComponent(m[2]));}catch(e){}}b.textContent=downloaded?'✓ Salvo':'⬇ Baixar capítulo';b.onclick=function(ev){ev.preventDefault();ev.stopPropagation();if(wid&&wt)sessionStorage.setItem('mpNativeWorkTitle:'+wid,wt);sessionStorage.setItem('mpNativePendingDownload',h);location.hash=h;};parent.appendChild(b);});}"
            + "function checkPending(){var q=sessionStorage.getItem('mpNativePendingDownload');if(q&&q===location.hash&&chapterInfo()){sessionStorage.removeItem('mpNativePendingDownload');setTimeout(function(){waitAndDownload(0);},700);}}"
            + "window.MPScanNativeRefresh=function(){hideWholeWorkDownloads();rememberWorkTitle();addGlobalButton();addChapterButton();addWorkButtons();checkPending();};"
    ...
    private class OfflineBridge {
        ...
        public void openDownloads() {
            runOnUiThread(MainActivityV2.this::showOfflineLibrary);
        }
        ...
        @JavascriptInterface
        public void downloadChapter(String workId, String chapterId, String workTitle, String chapterTitle, String pagesJson) {
            new Thread(() -> downloadChapterInternal(workId, chapterId, workTitle, chapterTitle, pagesJson)).start();
        }
    }

    private void downloadChapterInternal(String workId, String chapterId, String workTitle, String chapterTitle, String pagesJson) {
        File tmp = null;
        try {
            ...
            meta.put("workTitle", workTitle == null || workTitle.trim().isEmpty() ? "Obra MP SCAN" : workTitle.trim());
            meta.put("title", chapterTitle == null || chapterTitle.trim().isEmpty() ? "Capítulo MP SCAN" : chapterTitle.trim());
            ...
            sendJsDone("Capítulo salvo! Ele já está na aba da obra em Capítulos baixados. ✓");
        } catch (Exception e) {
    ...
    private void showOfflineLibrary() {
        ...
        for (int g = 0; g < groups.size(); g++) {
            OfflineGroup group = groups.get(g);
            totalChapters += group.chapters.size();
            String id = "work-" + g;
            tabs.append("<button class='work-tab").append(g == 0 ? " active" : "").append("' data-target='").append(id).append("'>")
                .append("<span>").append(html(group.title)).append("</span><b>").append(group.chapters.size()).append("</b></button>");

            panels.append("<section class='work-panel").append(g == 0 ? " active" : "").append("' id='").append(id).append("'>")
                .append("<div class='work-head'><div><small>OBRA</small><h2>").append(html(group.title)).append("</h2></div><span>")
                .append(group.chapters.size()).append(group.chapters.size() == 1 ? " capítulo" : " capítulos").append("</span></div><div class='chapter-list'>");

            for (OfflineItem item : group.chapters) {
                JSONObject m = item.meta;
                int pages = m.optInt("pageCount", 0);
                totalPages += pages;
                String chapterTitle = m.optString("title", "Capítulo MP SCAN");
                ...
                panels.append("<article class='chapter-card'><div class='chapter-icon'>📖</div><div class='chapter-copy'><strong>")
                    .append(html(chapterTitle)).append("</strong><span>").append(pages).append(" páginas")
                    .append(date.isEmpty() ? "" : " • salvo em " + html(date)).append("</span></div><div class='chapter-actions'>")
        ...
        String content;
        if (groups.isEmpty()) {
            content = "<div class='empty'><div>📥</div><h2>Nenhum capítulo baixado</h2><p>Abra uma obra no aplicativo e toque em <b>Baixar capítulo</b>. Cada obra ganhará sua própria aba aqui.</p></div>";
        } else {
            content = "<div class='tabs'>" + tabs + "</div><div class='panels'>" + panels + "</div>";
        }

        String page = "<!doctype html><html><head><meta name='viewport' content='width=device-width,initial-scale=1,viewport-fit=cover'>"
            + "<style>*{box-sizing:border-box}body{margin:0;background:#0d0912;color:#fff;font-family:system-ui,-apple-system,sans-serif;padding:22px 16px 42px}.top{max-width:820px;margin:0 auto 18px}.eyebrow{color:#d893ff;font-weight:900;font-size:12px;letter-spacing:.12em}.top h1{font-size:30px;margin:6px 0}.top p{color:#bcaecb;margin:0;line-height:1.5}.stats{display:flex;gap:9px;margin-top:15px;overflow:auto}.stat{background:#181020;border:1px solid #30223d;border-radius:16px;padding:11px 14px;min-width:106px}.stat b{display:block;font-size:19px}.stat span{color:#a995b9;font-size:11px}.back{display:inline-flex;margin-top:14px;text-decoration:none;color:#fff;background:#1c1325;border:1px solid #392747;padding:10px 14px;border-radius:14px;font-weight:800;font-size:12px}.tabs{max-width:820px;margin:0 auto 14px;display:flex;gap:9px;overflow-x:auto;padding:2px 0 7px;scrollbar-width:none}.tabs::-webkit-scrollbar{display:none}.work-tab{flex:0 0 auto;max-width:230px;border:1px solid #352542;background:#17101f;color:#c9bad5;border-radius:16px;padding:10px 12px;display:flex;align-items:center;gap:9px;font-weight:850;cursor:pointer}.work-tab span{max-width:175px;white-space:nowrap;overflow:hidden;text-overflow:ellipsis}.work-tab b{display:grid;place-items:center;min-width:24px;height:24px;border-radius:999px;background:#251831;color:#dcb1ff;font-size:11px}.work-tab.active{background:linear-gradient(135deg,#8f4ac7,#d85aa7);color:#fff;border-color:transparent;box-shadow:0 12px 30px rgba(143,74,199,.24)}.work-tab.active b{background:rgba(255,255,255,.18);color:#fff}.panels{max-width:820px;margin:auto}.work-panel{display:none}.work-panel.active{display:block}.work-head{display:flex;justify-content:space-between;gap:12px;align-items:flex-end;margin:4px 2px 12px}.work-head small{display:block;color:#a981c4;font-size:10px;font-weight:900;letter-spacing:.12em}.work-head h2{margin:3px 0 0;font-size:20px}.work-head>span{color:#ad9abc;font-size:11px;background:#17101f;border:1px solid #31223d;padding:7px 10px;border-radius:999px;white-space:nowrap}.chapter-list{display:grid;gap:11px}.chapter-card{display:grid;grid-template-columns:50px minmax(0,1fr);gap:12px;background:linear-gradient(145deg,#1a1124,#130d1b);border:1px solid #342342;border-radius:21px;padding:13px;box-shadow:0 14px 32px rgba(0,0,0,.2)}.chapter-icon{width:50px;height:50px;border-radius:15px;display:grid;place-items:center;background:#251831;border:1px solid #3c2850;font-size:21px}.chapter-copy{min-width:0}.chapter-copy strong{display:block;font-size:14px;line-height:1.35}.chapter-copy span{display:block;color:#a995b9;font-size:11px;margin-top:5px}.chapter-actions{grid-column:1/-1;display:flex;gap:8px}.chapter-actions a{text-decoration:none;text-align:center;flex:1;border-radius:13px;padding:10px 12px;font-weight:900;font-size:12px}.read{background:linear-gradient(135deg,#8f4ac7,#d85aa7);color:#fff}.del{background:#21172b;color:#d7c6e2;border:1px solid #3a2948}.empty{max-width:580px;margin:55px auto;text-align:center;background:#17101f;border:1px solid #30223d;border-radius:24px;padding:30px 20px}.empty>div{font-size:42px}.empty p{color:#a995b9;line-height:1.5}@media(max-width:520px){body{padding:18px 12px 36px}.top h1{font-size:26px}.work-tab{max-width:205px}.work-head{align-items:flex-start;flex-direction:column}.work-head h2{font-size:18px}}</style></head><body>"
            + "<section class='top'><div class='eyebrow'>MP SCAN • APLICATIVO</div><h1>Capítulos baixados</h1><p>O download é feito capítulo por capítulo. Cada obra fica separada em sua própria aba para deixar tudo organizado.</p>"
            + "<div class='stats'><div class='stat'><b>" + groups.size() + "</b><span>obras</span></div><div class='stat'><b>" + totalChapters + "</b><span>capítulos</span></div><div class='stat'><b>" + totalPages + "</b><span>páginas</span></div></div>"
            + "<a class='back' href='mpscan-offline://online'>← Voltar para o MP SCAN</a></section>" + content
            + "<script>document.querySelectorAll('.work-tab').forEach(function(b){b.addEventListener('click',function(){document.querySelectorAll('.work-tab').forEach(function(x){x.classList.remove('active')});document.querySelectorAll('.work-panel').forEach(function(x){x.classList.remove('active')});b.classList.add('active');var p=document.getElementById(b.getAttribute('data-target'));if(p)p.classList.add('active');});});</script></body></html>";
    ...
    public void onBackPressed() {
        if ("reader".equals(nativeScreen)) { showOfflineLibrary(); return; }
        if ("library".equals(nativeScreen)) { nativeScreen = ""; web.loadUrl(HOME); return; }

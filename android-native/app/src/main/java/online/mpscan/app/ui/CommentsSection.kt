...
private class API{val adminIds=setOf("eHLv7TlUOAW5rLsMwWVCGeu1KSI2","pc87zkEpz0Ra7HfKvRIwLkbl5K13");val b="https://nnnsss-23f2f-default-rtdb.firebaseio.com";fun e(v:String)=URLEncoder.encode(v,"UTF-8");fun path(t:String,w:String,c:String)=if(t=="obra")"comentariosV1/obra/${e(w)}" else "comentariosV1/capitulo/${e(w)}/${e(c)}";fun q(url:String,m:String="GET",body:String?=null):JSONObject{val x=URL(url).openConnection() as HttpURLConnection;x.requestMethod=m;x.connectTimeout=15000;x.readTimeout=25000;x.setRequestProperty("Content-Type","application/json");if(body!=null){x.doOutput=true;x.outputStream.use{it.write(body.toByteArray())}};val ok=x.responseCode in 200..299;val s=(if(ok)x.inputStream else x.errorStream)?.bufferedReader()?.use{it.readText()}.orEmpty();x.disconnect();if(!ok)error(if("TOKEN_EXPIRED" in s)"Sua sessão expirou. Entre novamente." else "Não foi possível conectar.");return if(s.isBlank()||s=="null")JSONObject()else JSONObject(s)}
 ...
 suspend fun react(t:String,w:String,c:String,id:String,emoji:String,s:AccountSession)=withContext(Dispatchers.IO){q("$b/${path(t,w,c)}/${e(id)}/reacoes/${e(s.uid)}.json?auth=${e(s.token)}","PUT",JSONObject().put("tipo",emoji).put("data",System.currentTimeMillis()).toString())}
 suspend fun reply(t:String,w:String,c:String,id:String,text:String,s:AccountSession)=withContext(Dispatchers.IO){q("$b/${path(t,w,c)}/${e(id)}/respostas.json?auth=${e(s.token)}","POST",JSONObject().put("uid",s.uid).put("texto",text).put("data",System.currentTimeMillis()).put("spoiler",false).toString())}
}
...
@Composable fun CommentsSection(type:String,workId:String,chapterId:String="",modifier:Modifier=Modifier){val ctx=LocalContext.current;val api=remember{API()};val store=remember{AccountStore(ctx)};val accounts=remember{AccountRepository()};val scope=rememberCoroutineScope();var session by remember{mutableStateOf(store.session())};var list by remember{mutableStateOf<List<CM>>(emptyList())};var text by remember{mutableStateOf("")};var sp by remember{mutableStateOf(false)};var busy by remember{mutableStateOf(false)};var err by remember{mutableStateOf("")};var login by remember{mutableStateOf(false)};var viewerAdmin by remember{mutableStateOf(false)}
 ...
 onReact={emoji->scope.launch{session?.let{runCatching{api.react(type,workId,chapterId,x.id,emoji,it)}.onSuccess{load()}.onFailure{err="Entre na conta para reagir."}}}},
 onReply={value->scope.launch{session?.let{runCatching{api.reply(type,workId,chapterId,x.id,value,it)}.onSuccess{load()}.onFailure{err="Não foi possível responder."}}}}
)}}}
@Composable private fun Card(x:CM,viewerUid:String,viewerAdmin:Boolean,onEdit:(String)->Unit,onDelete:()->Unit,onPin:()->Unit,onReact:(String)->Unit,onReply:(String)->Unit){var replying by remember(x.id){mutableStateOf(false)};var replyText by remember(x.id){mutableStateOf("")};var editing by remember(x.id){mutableStateOf(false)};var editText by remember(x.id){mutableStateOf(x.text)};var confirmDelete by remember(x.id){mutableStateOf(false)};val canManage=viewerUid.isNotBlank()&&(viewerUid==x.uid||viewerAdmin);
   ...
   Column(Modifier.padding(16.dp)){
    ...
    Row(Modifier.fillMaxWidth().horizontalScroll(rememberScrollState()).padding(top=10.dp),horizontalArrangement=Arrangement.spacedBy(7.dp)){
     listOf("👍","❤️","😂","😮","😢","🔥").forEach{emoji->
      var count=0;var selected=false;x.reactions.keys().forEach{uid->val value=x.reactions.opt(uid);val kind=if(value is JSONObject)value.optString("tipo",value.optString("reaction")) else value?.toString().orEmpty();if(kind==emoji){count++;if(uid==viewerUid)selected=true}}
      Surface(Modifier.clickable(enabled=viewerUid.isNotBlank()){onReact(emoji)},color=if(selected)MpAccent.copy(alpha=.75f) else Color.Black.copy(alpha=.35f),shape=CircleShape,border=BorderStroke(1.dp,Color.White.copy(alpha=.25f))){Text(if(count>0)"$emoji $count" else emoji,Modifier.padding(horizontal=10.dp,vertical=7.dp))}
     }
     TextButton({if(viewerUid.isNotBlank())replying=true}){Text("↩ Responder",color=Color(0xff75b8ff))}
     if(viewerAdmin)TextButton(onPin){Text("📌 Fixar",color=Color(0xffffb64d))}
    }
    if(x.replies.length()>0)Column(Modifier.padding(top=8.dp)){x.replies.keys().forEach{rid->x.replies.optJSONObject(rid)?.let{r->Surface(Modifier.fillMaxWidth().padding(top=5.dp),color=Color.Black.copy(alpha=.28f),shape=RoundedCornerShape(12.dp)){Text("↳ "+r.optString("texto","Resposta"),Modifier.padding(10.dp),color=Color.White)}}}}
 ...
 if(replying)AlertDialog(onDismissRequest={replying=false},title={Text("Responder comentário")},text={OutlinedTextField(replyText,{replyText=it.take(3000)},Modifier.fillMaxWidth())},confirmButton={Button({onReply(replyText.trim());replyText="";replying=false},enabled=replyText.isNotBlank()){Text("Responder")}},dismissButton={TextButton({replying=false}){Text("Cancelar")}})

...
@Composable private fun Card(x:CM,viewerUid:String,viewerAdmin:Boolean,onEdit:(String)->Unit,onDelete:()->Unit,onPin:()->Unit){var editing by remember(x.id){mutableStateOf(false)};var editText by remember(x.id){mutableStateOf(x.text)};var confirmDelete by remember(x.id){mutableStateOf(false)};val canManage=viewerUid.isNotBlank()&&(viewerUid==x.uid||viewerAdmin);

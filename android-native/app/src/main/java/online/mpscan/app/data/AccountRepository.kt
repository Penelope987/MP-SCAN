...
class AccountRepository{private val base="https://nnnsss-23f2f-default-rtdb.firebaseio.com";private val key="AIzaSyAbpqQIxWuEnFolv3lNjNDoPKTGm0mtrxU";private fun e(v:String)=URLEncoder.encode(v,"UTF-8")
 ...
 suspend fun profile(s:AccountSession)=withContext(Dispatchers.IO){
  ...
  val pub=runCatching{req("$base/perfisPublicos/${e(s.uid)}.json")}.getOrDefault(JSONObject())
  fun value(vararg keys:String):String{for(key in keys){u.optString(key).takeIf{it.isNotBlank()}?.let{return it};pub.optString(key).takeIf{it.isNotBlank()}?.let{return it}};return ""}
  val followers=count("$base/seguidores/${e(s.uid)}.json?auth=${e(s.token)}")
  val following=count("$base/seguindo/${e(s.uid)}.json?auth=${e(s.token)}")
  AccountProfile(s.uid,value("nome","name").ifBlank{"Leitor MP SCAN"},value("nomeUsuario","username"),value("bio"),value("foto","photo"),value("capaPerfil","cover"),value("corPerfil","color").ifBlank{"#8d2bff"},if(u.has("publico"))u.optBoolean("publico") else pub.optBoolean("publico",true),value("molduraComentarioId"),value("papel","role").ifBlank{"Usuário"},followers,following,countOwn(s.uid))
 }
